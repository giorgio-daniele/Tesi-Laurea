...
class Sample():
    ...
    def __init__(self, number: int, device: str, shark: Wireshark, skygo: Dazn, trace: str):
    ...
    def start_experiment(self):
        ...
        if not self.skygo.start():
            ...
            print("Error on starting DAZN application")
            self.shark.stop()
...
def main(number, repetitions, platform, interface, results_folder_name):
    ...
    for i in range(repetitions):
        ...
        service = Dazn(exe_path=skygo_exe_path, platform=platform, img_path=images_path)

        # Generate a new instance of Experiment controller
        sample = Sample(number=number, device=platform, shark=sniffer, skygo=service, trace=streambot_file_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--platform', type=str, default='desktop', choices=['desktop', 'mobile'], help='Platform type')
